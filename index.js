...
const totalContributions = GAMES_JSON.reduce( (acc, currentValue) => acc + currentValue.backers, 0);
...
function filterUnfundedOnly() {
    ...
    let unfundedGames = GAMES_JSON.filter( (unfundedValue) => unfundedValue.pledged < unfundedValue.goal)
...
function filterFundedOnly() {
    ...
    let fundedGames = GAMES_JSON.filter( (fundedValue) => fundedValue.pledged > fundedValue.goal)
...
let unfundedCount = GAMES_JSON.filter( (unfundedValue) => unfundedValue.pledged < unfundedValue.goal).length

// create a string that explains the number of unfunded games using the ternary operator
const displayStr = `
    A total of $${totalRaised.toLocaleString()} has been raised for ${GAMES_JSON.length} game(s). 
    Currently, ${unfundedCount} games remain unfunded. We need your help!
`;

// create a new DOM element containing the template string and append it to the description container
const descriptionText = document.createElement('p');
descriptionText.textContent = displayStr;
